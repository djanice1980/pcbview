...
void Renderer::createSceneTargets() {
    ...
    // cannot ask for a zero-sized or device-limit-busting image. The scale is
    // whatever the caller set -- the app picks it PER MODE on the CPU device
    // (raster native, traced reduced), because the reduced scale is a big win
